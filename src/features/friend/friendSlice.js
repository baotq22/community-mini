...
const slice = createSlice({
  ...
  reducers: {
    ...
    getFriendsSentSuccess(state, action) {
      state.isLoading = false;
      state.error = null;
...
export const getUsers = ({ filterName, page = 1, limit = USER_PER_PAGE }) => async (dispatch) => {
...
export const getFriends = ({ filterName, page = 1, limit = USER_PER_PAGE }) => async (dispatch) => {
  dispatch(slice.actions.startLoading());
  try {
    const params = { page, limit };
    if (filterName) params.name = filterName;
...
export const getFriendsRequests = ({ filterName, page = 1, limit = USER_PER_PAGE }) => async (dispatch) => {
  dispatch(slice.actions.startLoading());
  try {
    const params = { page, limit };
    if (filterName) params.name = filterName;
...
export const getFriendsSent = ({ filterName, page = 1, limit = USER_PER_PAGE }) => async (dispatch) => {
  dispatch(slice.actions.startLoading());
  try {
    const params = { page, limit };
    if (filterName) params.name = filterName;
    const response = await apiService.get("/friends/requests/outgoing", { params });
    dispatch(slice.actions.getFriendsSentSuccess(response.data));
  } catch (error) {
    dispatch(slice.actions.hasError(error.message));
    toast.error(error.message);
  }
